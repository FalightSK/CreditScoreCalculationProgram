...
from customerInfo import order
from newCustomer import extract_fin_info, read_file
...
for i, row in enumerate(doc.iloc()):
    ...
    if row['Transaction Status'] != 'สำเร็จ' or row.isnull().values.sum() > 3 or row['Transaction Value'] == 0:
        continue
